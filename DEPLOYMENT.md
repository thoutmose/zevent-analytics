...
### 6. `.env.postgres` on srv-prod

CD applies every file in `sql/` on each deploy (see below) by running
`psql` against srv-db from a throwaway `postgres:16` container — it needs
the same connection details already sitting in the main `.env` (`POSTGRES_HOST`,
`PGBOUNCER_PORT`, `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD` — see
[`ARCHITECTURE.md`](ARCHITECTURE.md)), but `deploy` must never read that file
(same reasoning as `.env.nifi` above — it also holds the Twitch secrets).
Same fix: a **separate** file, readable by `deploy` alone, containing only
what this one step needs. `psql`/`libpq` read these exact variable names
natively, no connection string needed:

```bash
# On srv-prod:
cat > /home/thoutmose/twitch-analytics-prod/.env.postgres <<'EOF'
PGHOST=<same value as POSTGRES_HOST in the main .env>
PGPORT=<same value as PGBOUNCER_PORT in the main .env>
PGDATABASE=zevent
PGUSER=<same value as POSTGRES_USER in the main .env>
PGPASSWORD=<same value as POSTGRES_PASSWORD in the main .env>
EOF
sudo chown <checkout owner>:twitch-deploy /home/thoutmose/twitch-analytics-prod/.env.postgres
chmod 640 /home/thoutmose/twitch-analytics-prod/.env.postgres   # owner rw, group r, other none
```

`docker run --env-file .env.postgres` (not `-e KEY=value`, which would put
the password on the command line, visible to anything reading `ps aux` on
srv-prod while the container starts) is how CD passes these through — see
`cd.yml`'s "Apply database migrations" step.

### 7. Sudoers grant for running dbt

`run_dbt.sh` (synced by CD, see below) needs `dbt/`'s `profiles.yml`
(`POSTGRES_HOST`/`PGBOUNCER_PORT`/`POSTGRES_DB`/`POSTGRES_USER`/
`POSTGRES_PASSWORD`) — which means running as `thoutmose`, the main `.env`'s
owner, not as `deploy`. Same pattern as the `uv sync`/`systemctl restart`
grants already referenced in `cd.yml`'s "Refresh deps & restart zevent
services" step comment (see `/etc/sudoers.d/twitch-deploy`) — add one more
narrow, command-specific line rather than anything broader:

```
# /etc/sudoers.d/twitch-deploy — add alongside the existing uv sync /
# systemctl restart lines:
deploy ALL=(thoutmose) NOPASSWD: /home/thoutmose/twitch-analytics-prod/run_dbt.sh
```

`visudo -c` after editing, as always with sudoers. This is the only new
grant this dbt work needed — no new users, no new SSH keys, no broadened
access to `deploy`. The [`dbt (manual)`](../.github/workflows/dbt.yml)
workflow (`workflow_dispatch` only — see its own header comment for why it's
never wired into any automatic trigger) is what actually invokes
`sudo -u thoutmose run_dbt.sh` over the same SSH connection CD already uses.
...
4. A second `rsync` syncs the Python source files (including
   `emote_catalog.py` and `run_dbt.sh`), `sql/`, `dbt/` (excluding its own
   build artifacts — `target/`, `dbt_packages/`, `logs/`, `.user.yml`, all
   regenerated by `dbt deps`/`dbt build` on srv-prod itself), and dependency
   files (`pyproject.toml`/`uv.lock`) the same way. This lands the *code*
   only — nothing here runs dbt (see "Sudoers grant for running dbt" above
   and the separate [`dbt (manual)`](../.github/workflows/dbt.yml) workflow).
5. Every file in `sql/` is applied against srv-db with `psql` (via a
   throwaway `postgres:16` container — see ".env.postgres on srv-prod"
   above), on every deploy, migration or not — safe only because every file
   in `sql/` is written to be idempotent (`CREATE TABLE IF NOT EXISTS`/`ADD
   COLUMN IF NOT EXISTS`).
6. `docker compose pull && docker compose up -d --remove-orphans`, then an
   **unconditional** `docker compose restart nifi` on srv-prod — not just
   relying on `up -d`, which only recreates a container whose own
   image/config changed, which a SQL-only migration never does. The
   restart exists specifically so `PutDatabaseRecord`'s cached table schema
   can never go stale after step 5 — see the incident this fixes for good
   in `sql/003`'s header comment and
   [`ARCHITECTURE.md`, step 6a](ARCHITECTURE.md#6a-putdatabaserecord--insert-branch-insert-relationship).
7. A health check polls `https://localhost:8443/nifi/` on srv-prod for up to
   ~100s and fails the deploy if NiFi doesn't come back up (expected to
   trip on every deploy now, given step 6 above restarts it unconditionally).
8. `uv sync` plus a restart of the three `zevent-*-prod` systemd units (see
   "Running as a service" in README.md).

**What it still deliberately doesn't do:** touch the NiFi flow's
processors/routing shape (`RouteOnAttribute`'s conditions,
`PutDatabaseRecord`'s table-name/update-keys expressions, adding a new
branch entirely) — that's still built and maintained by hand in the NiFi UI
(see [`ARCHITECTURE.md`](ARCHITECTURE.md)), not by this repo's CI/CD. A
change to `flow-templates/zevent-ingest-flow.json` (like the `emote_catalog`
UPSERT branch added alongside `sql/005_emote_catalog.sql`) has to be applied
to the live flow by hand, on both srv-dev and srv-prod, separately from a
normal deploy.
...
**Actions → dbt (manual) → Run workflow.** Not gated by the `production`
environment approval step CD uses — dbt only reads `bronze_*` and writes to
its own `stg`/`int`/`marts` schemas, never touching what `main.py`/NiFi
actually serve, so a bad run rebuilds some analytical tables, not
production-facing infrastructure. `workflow_dispatch` alone (a human with
repo write access clicking the button) is the trigger; there's no push, PR,
schedule, or `workflow_run` wired to it at all — see the workflow file's own
header comment for why that's deliberate. Whatever's currently on disk in
`dbt/`/`sql/` on srv-prod (i.e. whatever the last CD deploy landed) is what
runs; this workflow doesn't sync anything itself.
